...
class Market_DataLoader_srcpose():
    ...
    def __getitem__(self, index):
        ...
        src_path_mask = src_path.split('.')[0] + '.png'
        tgt_path_mask = tgt_path.split('.')[0] + '.png'
        ...
        src_pose = self.loader(os.path.join(self.pose_path, src_path))
        src_img_mask = self.loader(os.path.join(self.mask_path, src_path_mask))
        tgt_img_mask = self.loader(os.path.join(self.mask_path, tgt_path_mask))
        src_img, tgt_img, pose, src_pose, src_img_mask, tgt_img_mask = self.transform(src_img), self.transform(tgt_img), self.transform(pose), self.transform(src_pose), self.transform_mask(src_img_mask), self.transform_mask(tgt_img_mask)

        
        src_img_mask = src_img_mask.ge(0.5)

        tgt_img_mask = tgt_img_mask.ge(0.5)

        return src_img, tgt_img, pose, src_pose, src_img_mask, tgt_img_mask
...
class Market_test_srcpose():
    def __init__(self, imgs_name, imgs_path, pose_path, mask_path, transform, loader):
        # images
        images = imgs_name
        poses = os.listdir(pose_path)
    ...
    def __getitem__(self, index):
        ...
        src_img, pose, src_img_mask = self.transform(src_img), self.transform(pose), self.transform_mask(src_img_mask)

        return src_img, pose, src_img_mask, name
